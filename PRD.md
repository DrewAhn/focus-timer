...
[초기 상태] → Focus 모드 (40:00)
     ↓
[Start 클릭] → 타이머 카운트다운 시작 + 브라우저 탭 타이틀 업데이트
     ↓
[Pause 클릭] → 타이머 일시정지 (다시 Start로 재개)
     ↓
[Reset 클릭] → 현재 모드의 초기 시간으로 복귀
     ↓
[Focus 종료] → 종소리 알림 🔔 + 타이머 흔들림 + 진동(모바일)
     ↓
[2초 대기] → 자동으로 Break 모드 (10:00)로 전환
     ↓
[Break 종료] → 종소리 알림 🔔 + 타이머 흔들림 + 진동(모바일)
     ↓
[2초 대기] → 자동으로 Focus 모드 (40:00)로 전환 (사이클 반복)
     ↓
[사운드 OFF] → 타이머 흔들림 + 진동(모바일)만 실행, 자동 전환은 동일하게 작동
```
...
### 시나리오 1: 기본 포모도로 사이클 (자동 전환)
1. 사용자가 페이지 접속 → Focus 모드 (**40:00**) 표시
2. [Start] 클릭 → 타이머 시작, 브라우저 탭에 "40:00 - Focus Timer" 표시
3. **40분 후** 타이머 종료 → 종소리 🔔 + 타이머 흔들림 + "Switching to break..." 메시지
4. **2초 후** 자동으로 Break 모드 (**10:00**)로 전환 + "Break time! Take a rest ☕" 메시지
5. [Start] 클릭 → 휴식 타이머 시작, 탭에 "10:00 - Break Timer" 표시
6. **10분 후** 타이머 종료 → 종소리 🔔 + 타이머 흔들림 + "Switching to focus..." 메시지
7. **2초 후** 자동으로 Focus 모드 (**40:00**)로 전환 + "Ready to focus again! 💪" 메시지
8. 이후 사이클 자동 반복 (Start 버튼만 누르면 계속 진행)
...
- ✅ 자동 모드 전환 (Focus ↔ Break 사이클)
- ✅ 원형 프로그레스 바
- ✅ 브라우저 탭 타이틀에 실시간 시간 표시
- ✅ Web Audio API 종소리 알림
- ✅ Fallback 시각적 알림
- ✅ 사운드 ON/OFF 토글
- ✅ localStorage 설정 저장
- ✅ 모바일 진동 알림
...
### v2.1 (2025년 12월 17일) - 현재 버전 🎉

**주요 업데이트:**
- 🔄 **자동 모드 전환 기능 추가**
  - Focus 종료 시 2초 후 자동으로 Break 모드로 전환
  - Break 종료 시 2초 후 자동으로 Focus 모드로 전환
  - 완전 자동화된 포모도로 사이클
